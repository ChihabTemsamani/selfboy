...
// removed clt.removeAllListeners();
clt.on('ready',()=>{
...
clt.on("message",msg=>{
	try {
		...
		// removed global modifier on regex
		if (msg.author.id==clt.user.id) {
		// replaced .tag with .id
			if (/^!!kill/i.test(msg.content)) {
				if (/^!!killrest$/i.test(msg.content)) {
					msg.channel.send("*Bot restarting...*").then(clt.destroy()).then(()=>clt.login(token)).catch(process.exit);
					// oh my god dont put the token smh
					// replaced clt.user.destroy with clt.destroy()
				} else {
					msg.channel.send("*Bot shutdown...*").then(clt.user.destroy).then(process.exit).catch(process.exit);
				}
			} else if (/^!!sd /gmi.test(msg.content)) {
				msg.channel.send(msg.content.replace(/^!!sd /gi,"")).then(msg=>msg.delete(2500));
				// .delete() accepts one argument which is timeout, replaced setTimeout() with that
				msg.delete();
				return;
